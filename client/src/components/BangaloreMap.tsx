...
export default function BangaloreMap({ onSelectRegion }: BangaloreMapProps) {
  ...
  const controlsRef = useRef<OrbitControls | null>(null);
  const regionsRef = useRef<Mesh[]>([]);
  const raycasterRef = useRef<Raycaster>(new THREE.Raycaster() as unknown as Raycaster);
  const mouseRef = useRef<Vector2>(new THREE.Vector2() as unknown as Vector2);
  //
  ...
  useEffect(() => {
    ...
    regions.forEach((region, index) => {
      ...
      labelDiv.className =
        "absolute pointer-events-none py-1 px-2 bg-slate-800/80 text-white text-sm rounded-md hidden";
      ...
      if (index > 0) {
        ...
        createConnectionLine(
          ...
          new THREE.Vector3(
            prevRegion.position.x,
            prevRegion.position.y,
            prevRegion.position.z
          )
        );
      }
    });
    ...
    const handleMouseMove = (event: MouseEvent) => {
      ...
      if (intersects.length > 0) {
        ...
        if (intersectedObject.userData.label) {
          ...
          labelDiv.style.transform = `translate(-50%, -100%) translate(${x}px, ${
            y - 10
          }px)`;
        }
    ...
    <div className="relative w-full h-[300px] md:h-[400px] rounded-lg overflow-hidden">
      ...
      <div className="absolute bottom-3 left-3 text-xs bg-slate-800/80 text-slate-300 py-1 px-2 rounded">
        {selectedRegion ? (
          <span>
            Selected:{" "}
            {regions.find((r) => r.id === selectedRegion)?.name || "None"}
          </span>
        ) : (
